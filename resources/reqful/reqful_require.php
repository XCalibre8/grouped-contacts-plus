...
function GroupLinkCount($RQRFailError, $sql) {
	...
	if ($stm) {
		...
		else $RQRFailError .= 'Require fulfillments encountered an error counting group link types ('.$sql->error.').<br />';
	}
	else $RQRFailError .= 'Require fulfillments encountered an error preparing to count group link types ('.$sql->error.').<br />';
};

//XCal - We only want to pay attention to request variables if they're for the all links view page
if (isset($_REQUEST['For']) and ($_REQUEST['For'] == 'RQR')) {
	$For = 'RQR';
	//XCal - [XC:T|Target:String(The DOM Element ID of the container);;]
	if (isset($_REQUEST['T']))
		$RQRTarget = $_REQUEST['T'];

	//XCal - [XC:OLT|Owner Link Type: The owner link type ID;;]
	if (isset($_REQUEST['LTP']))
		$RQRLinkType = $_REQUEST['LTP'];
	if (isset($_REQUEST['ID']))
		$RQRID = $_REQUEST['ID'];
	
	if (isset($_REQUEST['REQ']))
		$RQRREQID = $_REQUEST['REQ'];
	if (isset($_REQUEST['ALT']))
		$RQRApplyLinkType = $_REQUEST['ALT'];
...
if (isset($RQRFLTID) and ($RQRFLTID == 'NULL'))
	$RQRFLTID = null;
if (isset($RQRApplyLinkType) and ($RQRApplyLinkType == 'NULL'))
	$RQRApplyLinkType = null;
...
if ($RQRDieError == '') {
	$RQRKeys = "For=RQR&T=$RQRTarget&LTP=$RQRLinkType&ID=$RQRID";
	...
	if ($TokenValid) {
		...
		$IsLinked = ItemLinked($sql,$RQRLinkType,$RQRID);
		...
		if (in_array($RQRMode,array('N','M','D'))) {
			if (in_array($RQRMode,array('M','D')) and (!($RQRREQID > 0))) {
					...
					if (isset($RQRCount)) { //XCal - If the type's been passed we know it's a save operation
						...
						if ($stm) {
							...
							if ($stm->execute()) {
								...
								if ($stm->prepare('CALL sp_rqf_addedit_requirement(@REQID,?,?,?,?,?,?,?,@Code,@Msg)')) {							
									$stm->bind_param('iiiiiii',$RQRLinkType,$RQRID,$RQRApplyLinkType,$RQRFLTID,$RQRFLMID,$RQRFLLID,$RQRCount);
									if ($stm->execute()) {
										$stm->free_result();
						...
						if ($RQRREQID > 0) {
							...
							if ($stm) {
								$stm->bind_param('i',$RQRREQID);
								if ($stm->execute()) {
									$stm->bind_result($RQRApplyLinkType,$RQRFLTID,$RQRFLMID,$RQRFLLID,$RQRCount,$RQRUpdated);
									$stm->fetch();
									$stm->free_result();
								}
						...
						echo('Requirement</h5>'.
							'<input id="REQ_ID" type="hidden" value="'.$RQRREQID.'" />'.
							'<input id="REQ_ONR_ID" type="hidden" value="'.$RQRID.'" />'.
							'<table><tr>'.
							'<td class="fieldname">For...</td>'.
							'<td><select id="REQ_FOR_LTP_ID" onchange="'.
								"SelectFulfillmentType('$RQRLinkType','$RQRID','REQ','require')".
								'">');
						if ($RQRLinkType == 4)
						...
						if ($stm) {
							if ($TCount > 0)
								$stm->bind_param('i',$RQRID);
							else $stm->bind_param('i',$RQRLinkType);
							if ($stm->execute()) {
								$stm->bind_result($LTPID,$LTPName);
								while ($stm->fetch()) {
									if (!isset($RQRApplyLinkType))
										$RQRApplyLinkType = $LTPID;
									echo('<option value="'.$LTPID.'"');
									if ($LTPID == $RQRApplyLinkType)
										echo(' selected');
									echo(">$LTPName</option>");
								}
								$stm->free_result();
						...
						echo('</div></td></tr>'.
							...
							'<input class="save" type="button" value="Save" onclick="'.
								"SaveRequirement('$RQRTarget','$RQRLinkType','$RQRMode'".');" />'.
							'<input class="cancel" type="button" value="Cancel" onclick="'.
							"AJAX('$RQRTarget','$RQRPath','$RQRKeys&M=L');\" /><br /><br />");
					}
				}
			...
			if ($CanView) {
				...
				if (isset($TCount) and ($TCount > 0)) {
					echo('<h4>Require Fulfillments</h4>'.				
						...
						"AJAX('$RQRTarget','$RQRPath','$RQRKeys&M=N');".	
						'" /><br />');
					...
					if ($stm) {
						$stm->bind_param('ii',$RQRLinkType,$RQRID);
						if ($stm->execute()) {
						...
						if ($stm) {					
							...
							if ($stm->execute()) {
								...
								while ($stm->fetch()) {
									...
									if ($RCount > 0)
										echo('&nbsp;<input type="button" class="button" value="Find" onclick="'.
									...
									if (is_null($FLTID) or ($FLTID == 0)) { //XCal - Just a min or max link count needed
										...
										if ($chk) {
											$chk->bind_param('iii',$RQRLinkType,$RQRID,$FLTForLTP);
											if ($chk->execute()) {
										...
										if ($chk) {
											$chk->bind_param('iiiiiii',$FLTID,$FLMID,$RQRLinkType,$RQRID,$FLTForLTP,$Level,$Level);
											if ($chk->execute()) {
												$chk->bind_result($FCount);
												$chk->fetch();
												$chk->free_result();
									...
									echo("<td>$FCount");
									if ($FCount > 0)
										echo('&nbsp;<input type="button" class="button" value="Show" onclick="'.
									...
									if ($CanMod)
										echo('<td><input type="button" class="button" value="Edit" onclick="'.
												"AJAX('$RQRTarget','$RQRPath','$RQRKeys&M=M&REQ=$REQID');".
												'" /></td>');
									if ($CanRem)
										echo('<td><input type="button" class="button" value="Remove" onclick="'.
												"AJAX('$RQRTarget','$RQRPath','$RQRKeys&M=D&REQ=$REQID');".
												'" /></td>');
									echo('</tr>');
